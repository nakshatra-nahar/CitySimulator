    ...
    StackTrace.fromError(error)
        .then((stackFrames) => {
            ...
            el.insertAdjacentHTML(
                "beforeend",
                `<h2>${prefix}: ${error.message}</h2>`
            );
            for (let frame of stackFrames) {
                const fileName = frame.fileName.replace(
                    window.location.origin,
                    ""
                );
                el.insertAdjacentHTML(
                    "beforeend",
                    `${frame.functionName} in ${fileName} ${frame.lineNumber}:${frame.columnNumber}<br/>`
                );
            }
...
declare module "../target/wasm32-unknown-unknown/release/cb_browser_ui" {
    ...
    type Intent = {
        Road?: { path: EditArcLinePath };
        Zone?: { boundary: EditArcLinePath };
    };
    ...
    export default interface CBRustAPI {
        ...
        insert_control_point(
            ...
            gestureId: string,
            point: [number, number],
            doneInserting: boolean
        );
        split_gesture(
            projectId: string,
            gestureId: string,
            point: [number, number],
            doneSplitting: boolean
        );
        set_n_lanes(
            projectId: string,
            gestureId: string,
            nLanesForward: number,
            nLanesBackward: number,
            doneChanging: boolean
        );
    }
}

type CBRustAPI =
    import("../target/wasm32-unknown-unknown/release/cb_browser_ui").default;
...
export type SetSharedState = (
    updater: (oldState: SharedState) => SharedState
) => void;
...
import("../target/wasm32-unknown-unknown/release/cb_browser_ui")
    .then((mod) => mod.default as CBRustAPI)
    .then((cbRustBrowser) => {
        ...
        const settingSpecs = {
            ...
            rendering: {
                retinaFactor: {
                    default: 2,
                    description: "Oversampling/Retina Factor",
                    min: 0.5,
                    max: 4.0,
                    step: 0.1,
                },
        ...
        class citysimulatorReactApp extends React.Component {
        ...
        window.cbReactApp = ReactDOM.render(
            <citysimulatorReactApp />,
            document.getElementById("layers-root")
        );
