...
function moveControlPoint(
    ...
    window.cbRustBrowser.move_gesture_point(
        projectId,
        gestureId,
        pointIdx,
...
export function ControlPoints({
    ...
    return (
        <>
            {Object.entries(
                gesturesFromAllRelevantPlans(state, currentProject)
            ).flatMap(([gestureId, gesture]) => {
                let isRelevant =
                    (gesture.intent.Road && planningMode === "roads") ||
                    (gesture.intent.Zone && planningMode === "zoning");
                if (!isRelevant) return [];
                const path = gesture.intent.Road
                    ? gesture.intent.Road.path
                    : gesture.intent.Zone
                    ? gesture.intent.Zone.boundary
                    : null;
                if (!path) return [];
                return [...path.corners.entries()].map(([pointIdx, corner]) => {
                    ...
                    return (
                        <ControlPointInteractable
                            ...
                            mesh={
                                state.planning.rendering.staticMeshes.GestureDot
                            }
                        />
                    );
                });
...
function gesturesFromAllRelevantPlans(
    state: SharedState,
    currentProject: string
): {
    ...
    return Object.keys(state.planning.master.gestures)
        .map((gestureId) => ({
            [gestureId]: Object.assign(
                {},
                state.planning.master.gestures[gestureId][0],
                { fromMaster: true }
            ),
        }))
        .concat(
            currentProject && state.planning.projects[currentProject]
                ? state.planning.projects[currentProject].undoable_history
                      .concat([
                          state.planning.projects[currentProject].ongoing || {
                              gestures: [],
                          },
                      ])
                      .map((step) => step.gestures)
                : []
        )
        .reduce((coll, gestures) => Object.assign(coll, gestures), {});
}

function ControlPointInteractable({
    point,
    isFirst,
    isLast,
    onMoved,
                ...
                onEvent={(e) => {
                    ...
                    if (e.drag) {
                        if (e.drag.end) {
                            if (
                                (isFirst || isLast) &&
                                vec3.dist(e.drag.end, e.drag.start) <
