...
import { SharedState, ToToolPortal } from "../citysimulator";
...
export function PlanningMenu({
    ...
    planningMode: "roads" | "zoning" | null;
    setPlanningMode: (mode: "roads" | "zoning" | null) => void;
}) {
        ...
        if (currentProject) {
            ...
            if (oldState.planning.canvasMode.currentGesture) {
                ...
                const lastHistoryStep =
                    project &&
                    project.undoable_history[
                        project.undoable_history.length - 2
                    ];
                if (
                    !lastHistoryStep ||
                    !lastHistoryStep.gestures[
                        oldState.planning.canvasMode.currentGesture
                    ]
                ) {
    ...
    return (
        <ToToolPortal>
            {currentProject ||
            Object.keys(state.planning.projects).length > 0 ? (
                <Select
                    style={{ width: 180 }}
                    showSearch={true}
                    ...
                    value={
                        state.planning.projects[currentProject]
                            ? currentProject
                            : currentProject
                            ? "Opening..."
                            : undefined
                ...
                >
                    {Object.keys(state.planning.projects).map((projectId) => (
                        <Option value={projectId}>
                            Project '{projectId.slice(0, 3).toUpperCase()}'
                        </Option>
                    ))}
                </Select>
            ) : (
                <Button type="primary" onClick={startNewProject}>
                    Start new project
                </Button>
            )}

            {currentProject && (
                <Button type="primary" onClick={implementProject}>
                    Implement
                </Button>
            )}

            <Toolbar
                id="planning-history-toolbar"
                options={{
                    undo: {
                        description: "Undo",
                        disabled:
            ...
            {currentProject && (
                <Toolbar
                    ...
                    onChange={(mode) => {
                        ...
                        if (mode === "roads") {
                            setIntent({
                                Road: cbRustBrowser.new_road_intent(1, 1),
                            });
                        } else {
                            setIntent(null);
                        }
                ...
                <Toolbar
                    ...
                    options={{
                        Residential: {
                            ...
                            color: toCSS(fromLinFloat(colors["Residential"])),
                        },
                        Commercial: {
                            description: "Commercial",
                            color: toCSS(fromLinFloat(colors["Commercial"])),
                        },
                        Industrial: {
                            description: "Industrial",
                            color: toCSS(fromLinFloat(colors["Industrial"])),
                        },
                        Agricultural: {
                            description: "Agricultural",
                            color: toCSS(fromLinFloat(colors["Agricultural"])),
                        },
                        Recreational: {
                            description: "Recreational",
                            color: toCSS(fromLinFloat(colors["Recreational"])),
                        },
                        Administrative: {
                            description: "Administrative",
                            color: toCSS(
                                fromLinFloat(colors["Administrative"])
                            ),
                        },
                    }}
                    ...
                    onChange={(newLandUse) =>
                        setIntent({
                            Zone: cbRustBrowser.new_zone_intent(newLandUse),
                        })
                    }
                />
            )}
