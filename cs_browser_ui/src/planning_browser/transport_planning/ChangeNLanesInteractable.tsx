...
function setNLanes(
    ...
    window.cbRustBrowser.set_n_lanes(
        projectId,
        gestureId,
        nLanesForward,
        nLanesBackward,
...
export function ChangeNLanesInteractable({
    gestureId,
    centerLine,
    nLanesBackward,
    nLanesForward,
        ...
        <>
            {hoverPoint && (
                <RenderLayer
                    ...
                    batches={[
                        {
                            mesh: state.planning.rendering.staticMeshes
                                .GestureChangeNLanes,
                ...
                onEvent={(e) => {
                    if (e.drag) {
                        if (e.drag.end || e.drag.now) {
                            ...
                            const orthogonalRightDirection = [
                                direction[1],
                                -direction[0],
                            ];
                            const vector = vec2.sub(
                                vec2.create(),
                                position,
                                projectedPosition
                            );
                            const orthogonalDistance = vec2.dot(
                                vector,
                                orthogonalRightDirection
                            );
                            let newNLanesForward = nLanesForward;
                            let newNLanesBackward = nLanesBackward;

                            if (orthogonalDistance > 0.0) {
                                newNLanesForward = Math.max(
                                    0.0,
                                    Math.round(orthogonalDistance / 3.0)
                                );
                            } else {
                                newNLanesBackward = Math.max(
                                    0.0,
                                    Math.round(-orthogonalDistance / 3.0)
                                );
                            }
                    ...
                    if (e.hover) {
                        if (e.hover.end) {
                            ...
                            let shiftedPoint;
                            if (orthogonalDistance > 0.0) {
                                shiftedPoint = vec2.scaleAndAdd(
                                    vec2.create(),
                                    e.hover.projectedPosition,
                                    orthogonalRightDirection,
                                    LANE_DISTANCE * nLanesForward
                                );
                            } else {
                                shiftedPoint = vec2.scaleAndAdd(
                                    vec2.create(),
                                    e.hover.projectedPosition,
                                    orthogonalRightDirection,
                                    -LANE_DISTANCE * nLanesBackward
                                );
                            }
                            setHoverPoint({
                                position: shiftedPoint,
                                direction,
                            });
                        }
                    }
